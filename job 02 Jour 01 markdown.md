job 02 Jour 01 markdown
Positionnez-vous dans le dossier du projet :
«welcome-to-docker »
● Consulter les fichiers présent comme Dockerfile
![alt text](file:///c%3A/Users/dell/Desktop/docker%20image/dockerfile.png)
● Analysez les et comprenez ce qu’il font
● Consulter le fichier README.md
![alt text](file:///c%3A/Users/dell/Desktop/docker%20image/docker%20readme.png)
● Créer l’image docker à partir de ce projet de sorte que le
fichier dockerfile soit pris en compte
![alt text](file:///c%3A/Users/dell/Desktop/docker%20image/construction%20image%20.png)
● Lancer l’image docker que vous venez de créer et lancer un
container avec
![alt text](file:///c%3A/Users/dell/Desktop/docker%20image/image%20lancer%20.png)
● Vérifiez s’il est lancé et d’autres commandes de bases que
vous avez faites dans le premier exercice
● Accéder au container pour visualiser le résultat
● Retournez dans Visual Studio et coder quelques lignes dans
votre projet
● Vérifiez le résultat faites en sorte que vos modifications soient
prises en compte dans votre image docker et votre container
![alt text](file:///c%3A/Users/dell/Desktop/docker%20image/docker%20image%20en%20fran%C3%A7ais.png)
● Comprenez ce qu’il faut faire pour que ce soit pris en compte
(modfier app.js)
● Publier sur votre compte docker une image docker, et rendez
la disponible à un membre de votre promo
![alt text](file:///c%3A/Users/dell/Desktop/docker%20image/docker%20hub%20.png)
● Récupérer une image docker d’un membre de votre promo et
refaites les tests et modifications sur celle-ci (citez l’auteur
de l’image d’origine dans la page)
![alt text](file:///c%3A/Users/dell/Desktop/docker%20image/docker%20image%20collegue%20promo.png)
